...
contract GameItem is ERC721URIStorage {
    ...
    function getContract(uint _index)
        public
        view
        returns (address from, address to, uint balance)
    {
        transferContract tContract = transferContracts[_index];

        return (tContract.from(), tContract.to(), address(tContract).balance);
    }	
...
contract transferContract {
        ...
        inState(State.Release)
    {
        ...
        GI.transfer(thisAddress, to, tokenId);
